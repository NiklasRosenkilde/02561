...
function render(time) {
  ...
  // ground perspective
  {
    var uLocation = gl.getUniformLocation(groundProgram, 'perspective');
    gl.uniformMatrix4fv(uLocation, false, flatten(perspectiveMatrix));
  }

  // ground modelView
  {
    var uLocation = gl.getUniformLocation(groundProgram, 'modelView');
  ...
  if (!g_drawingInfo) {
    ...
    var transY = moveTeapot ? 0.25 * Math.cos(t) : 0;
    ...
    // FIRST, THE SHADOWS
    {
      ...
      var uLocation = gl.getUniformLocation(teapotProgram, 'modelView');
      gl.uniformMatrix4fv(uLocation, false, flatten(modelView));

      // shadow mode
      var visLocation = gl.getUniformLocation(teapotProgram, 'visible');
    ...
    // THEN, THE TEAPOT
    {
      ...
      var uLocation = gl.getUniformLocation(teapotProgram, 'perspectiveMatrix');
      gl.uniformMatrix4fv(uLocation, false, flatten(perspectiveMatrix));
    }
